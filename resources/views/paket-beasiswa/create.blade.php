...
@section('title', 'Tambah Paket Beasiswa')

@section('content')
<div class="page-header">
    <h1 class="page-title">Tambah Paket Beasiswa</h1>
</div>

<div class="form-card">
    <form action="{{ route('paket-beasiswa.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="form-group">
            <label for="nama_beasiswa">Nama Beasiswa</label>
            <input type="text" name="nama_beasiswa" id="nama_beasiswa" placeholder="Masukkan nama beasiswa" value="{{ old('nama_beasiswa') }}" required>
            @error('nama_beasiswa')
        ...
        <div class="form-group" id="fase-container">
            ...
            @if(old('fase_checkpoint'))
                @foreach(old('fase_checkpoint') as $index => $val)
            ...
            @else
                <div class="dynamic-input-group" style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <input type="text" name="fase_checkpoint[]" placeholder="Contoh: Seleksi Berkas" value="" required style="flex: 1;">
                    <button type="button" class="btn btn-secondary add-fase-btn" style="padding: 0 16px; font-size: 20px;">+</button>
                </div>
            @endif
        ...
        <div class="form-group" id="persyaratan-container">
            ...
            @if(old('persyaratan'))
                @foreach(old('persyaratan') as $index => $val)
                    <div class="dynamic-input-group" style="display: flex; gap: 8px; margin-bottom: 8px;">
                        <input type="text" name="persyaratan[]" placeholder="Contoh: Mahasiswa Aktif Semester 3" value="{{ $val }}" required style="flex: 1;">
                        @if($index == 0)
            ...
            @else
                <div class="dynamic-input-group" style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <input type="text" name="persyaratan[]" placeholder="Contoh: Mahasiswa Aktif Semester 3" value="" required style="flex: 1;">
                    <button type="button" class="btn btn-secondary add-syarat-btn" style="padding: 0 16px; font-size: 20px;">+</button>
                </div>
            @endif
            @error('persyaratan')
        ...
        <div class="form-group" id="benefit-container">
            ...
            @else
                <div class="dynamic-input-group" style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <input type="text" name="benefit[]" placeholder="Contoh: Bantuan Biaya Hidup" value="" required style="flex: 1;">
                    <button type="button" class="btn btn-secondary add-benefit-btn" style="padding: 0 16px; font-size: 20px;">+</button>
                </div>
            @endif
            @error('benefit')
        ...
        <div class="form-group">
            ...
            <input type="url" name="url" id="url" placeholder="Masukkan URL Beasiswa" value="{{ old('url') }}" required>
            @error('url')
                <div class="error-text">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="harga" id="harga" placeholder="Masukkan harga" value="{{ old('harga') }}" min="0" required>
            @error('harga')
                <div class="error-text">{{ $message }}</div>
            @enderror
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.add-fase-btn').addEventListener('click', function() {
        ...
        // Insert before any error messages
